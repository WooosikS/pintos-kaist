...
initd (void *f_name) {
#ifdef VM
	supplemental_page_table_init (&thread_current ()->spt); // 301
#endif
	process_init ();
...
#ifdef VM
	  if(!hash_empty(&curr->spt.spt_table)){
...
lazy_load_segment (struct page *page, void *aux) {
	...
	off_t offset = ((struct file_info*)aux)->ofs;
	size_t page_read_bytes = ((struct file_info*)aux)->read_bytes;
	size_t page_zero_bytes = PGSIZE - page_read_bytes;
 ...
 * or disk read error occurs. 
 * 
 * 주소 UPage의 FILE에서 오프셋 OFS에서 시작하는 세그먼트를 로드합니다. 총 가상 메모리의 READ_BYTS + ZERO_BYTS 바이트는 다음과 같이 초기화됩니다.
		- UPAGE에서 READ_bytes 바이트는 오프셋 OFS에서 시작하는 FILE에서 읽어야 합니다.
		- UPAGE에서 ZERO_BYTS 바이트 + READ_BYTS를 0으로 설정해야 합니다.
		이 기능으로 초기화된 페이지는 쓰기 가능이 참이면 사용자 프로세스에 의해 쓰기 가능해야 하며,
		그렇지 않으면 읽기 전용이어야 합니다.
		메모리 할당 오류 또는 디스크 읽기 오류가 발생하면 true를 반환하고, false를 반환합니다
 * */
static bool
load_segment (struct file *file, off_t ofs, uint8_t *upage,
		uint32_t read_bytes, uint32_t zero_bytes, bool writable) {
	...
	while (read_bytes > 0 || zero_bytes > 0) {
		...
		// page 멤버들 설정, 가상페이지가 요구될 때 읽어야할 파일의 오프셋과 사이즈, 마지막에 패딩할 제로 바이트 등등
		file_info->file = file;
		file_info->ofs = ofs;
